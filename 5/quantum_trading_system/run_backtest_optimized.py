...
def generate_realistic_data(days=500):
    """生成更真实的测试数据（包含牛市、熊市、震荡）"""
    np.random.seed(42)
    
    dates = [datetime.now() - timedelta(days=days-i) for i in range(days)]
    
    # 模拟真实市场周期
    phase1 = np.linspace(100, 150, 150)  # 牛市
    phase2 = 150 + np.sin(np.linspace(0, 6*np.pi, 200)) * 20  # 震荡
    phase3 = np.linspace(150, 120, 150)  # 熊市
...
def main():
    ...
    # 优化后的策略列表
    strategies = [
        ("均线交叉策略(优化)", StrategyMaCross(short_period=10, long_period=60)),
        ("RSI基础策略(优化)", StrategyRsi(oversold=25, overbought=75)),
        ("RSI增强策略(优化)", StrategyRsiEnhanced(oversold=25, overbought=75, stop_loss_pct=0.05)),
        ("RSI自适应策略(优化)", StrategyRsiAdaptive()),
        ("布林带策略(优化)", StrategyBollinger(window=20, num_std=2.0)),
        ("多因子策略", StrategyMultiFactor()),
        ("网格策略(震荡)", StrategyGrid(grid_levels=8, grid_range_pct=0.08)),
        ("网格进阶策略(震荡)", StrategyGridAdvanced(grid_levels=8, grid_range_pct=0.08)),
        ("网格高频策略(优化)", StrategyGridScalping(tick_size=0.01, profit_target=0.003)),
    ]
    ...
    if results:
        ...
        print(f"{'排名':<6} {'策略名称':<20} {'收益率':<12} {'夏普比率':<10} {'最大回撤':<10} {'交易次数':<8} {'胜率':<8}")
        print("-"*80)
        
        for i, (_, row) in enumerate(results_df.iterrows(), 1):
            rank_icon = "🥇" if i == 1 else "🥈" if i == 2 else "🥉" if i == 3 else f"  {i}."
            print(f"{rank_icon:<6} {row['name']:<20} {row['total_return_pct']:>+.2f}%{''*4} {row['sharpe_ratio']:>6.2f}{' '*4} {row['max_drawdown_pct']:>6.2f}%{' '*4} {row['total_trades']:>8} {row['win_rate_pct']:>6.1f}%")
    ...
    print("="*80)
    print("\n💡 优化建议:")
    print("  1. RSI策略建议降低超卖阈值至25以增加信号")
    print("  2. 网格策略适合震荡市场，趋势市场需谨慎")
    print("  3. 均线交叉策略建议使用较长周期(10/60)")
    print("  4. 建议使用真实历史数据进行验证")
    print("  5. 考虑策略组合分散风险")
